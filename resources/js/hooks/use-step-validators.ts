...
function validateStep1(data: BookingFormData): Step1Errors {
  ...
  if (!phone || phone === "+63")  e.contactNumber = "Contact number is required.";
  else if (!PH_PHONE.test(phone)) e.contactNumber = "Enter a valid PH number (09XXXXXXXXX or +639XXXXXXXXX).";

  const age = String(data.age ?? "").trim();
  if (!age)                                       e.age = "Age is required.";
  else if (Number(age) < 1 || Number(age) > 120) e.age = "Age must be between 1 and 120.";

  if (!data.gender) e.gender = "Please select your biological sex.";
...
function validateStep2(data: BookingFormData): Step2Errors {
  ...
  if (!data.patientStatus) e.patientStatus = "Please select your patient status.";
  ...
  if (!data.appointmentDate) {
    ...
    if (chosen < tomorrow)  e.appointmentDate = "The appointment date must be at least tomorrow.";
    else if (chosen > maxDate) e.appointmentDate = `Appointments cannot be booked more than ${BOOKING_MAX_DAYS} days in advance.`;
  }
...
function validateStep3(data: BookingFormData): Step3Errors {
  ...
  if (!data.coverage) e.coverage = "Please select a mode of coverage.";

  if (data.coverage === "hmo") {
    if (!data.hmo)              e.hmo   = "Please select your HMO provider.";
    if (!data.hmoId)            e.hmoId = "Please enter your HMO ID number.";
    else if (data.hmoId.length < 6) e.hmoId = "HMO ID must be at least 6 characters.";
  }

  if (!data.appointmentTime) e.appointmentTime = "Please select a time slot.";
