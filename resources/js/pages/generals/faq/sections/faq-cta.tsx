// resources/js/pages/generals/faqs/sections/FaqsCtaSection.tsx
import { Link } from '@inertiajs/react';
import { useInView } from '@/hooks/useInView';
import { contact } from '@/routes';
import { faqsCtaData } from './faq-data';
...
export default function FaqsCtaSection() {
    const { ref, inView } = useInView();
    const { pill, heading, desc, ctas } = faqsCtaData;

    return (
        <section className="bg-[var(--wc-gray-50)] py-20">
            <div className="wc-container">
                <div
                    ref={ref}
                    className="relative overflow-hidden rounded-[var(--radius-4xl)] px-6 py-16 text-center transition-all duration-[600ms] md:px-12"
                    style={{
                        opacity: inView ? 1 : 0,
                        transform: inView
                        ...
                        background:
                            'linear-gradient(135deg, var(--wc-blue-700) 0%, var(--wc-blue-600) 40%, var(--wc-sky-500) 100%)',
                    }}
                >
                    {/* Radial glows */}
                    <div
              ...
              `,
                    ...
                    <div className="relative z-10 mx-auto max-w-[600px]">
                        {/* Badge */}
                        <span
                            className="inline-flex items-center rounded-full px-4 py-1.5 text-xs font-bold tracking-[var(--tracking-widest)] uppercase"
                            style={{
                        ...
                        <h2
                            className="mt-5 mb-5 text-[clamp(1.875rem,4vw,3rem)] tracking-[-0.04em]"
                            style={{ color: '#ffffff' }}
                        >
                            {heading.line1}
                            <br />
                        ...
                        <div className="flex flex-wrap justify-center gap-4">
                            {/* Internal route — use <Link> with wayfinder .url() */}
                            <Link
                                href={contact.url()}
