...
/**
 * One row is now a `loa_requests` record, not an appointment — `id` is the LOA
 * id and is what the approve/reject routes bind. The appointment fields are
 * still here because the officer decides against the visit, not the paperwork.
 */
...
function RejectModal({
    ...
    function submit(): void {
        ...
        setBusy(true);
        router.post(
            `/hr/hmo-approvals/${appointment.id}/reject`,
            { reason },
            {
                preserveScroll: true,
                onSuccess: () => {
                    onClose();
                    onSuccess(
                        `LOA ${appointment.loaNumber} for ${appointment.patient} has been rejected.`,
                    );
                },
                onError: () => {
                    setBusy(false);
                    onError('Failed to reject the LOA. Please try again.');
                },
            ...
            <td style={{ padding: 'var(--space-4) var(--space-5)' }}>
                ...
                >
                    ...
                    <div>
                        ...
                        {/* The reference the patient quotes at the counter. */}
                        <p
                            style={{
                                margin: '2px 0 0',
                                fontSize: 'var(--text-xs)',
...
export default function HmoApprovalsPage(): ReactElement {
    ...
    const [rejectTarget, setRejectTarget] = useState<HmoAppointment | null>(
        null,
    );
    ...
    const filtered = props.appointments.filter((a) => {
        ...
        const matchSearch =
            ...
            a.loaNumber.toLowerCase().includes(q) ||
    ...
    const hmoProviders = [
        ...new Set(
            props.appointments.map((a) => a.hmo).filter(Boolean) as string[],
        ),
    ];
        ...
        <HRDashboardLayout activeId="hmo-approvals">
            ...
            >
                <div>
                    ...
                    >
                        Verify HMO coverage before appointments reach the
                        doctor's queue
                    </p>
                </div>
            </div>
            ...
            >
                ...
                >
                    <strong>Approving</strong> an LOA forwards the appointment
                    to the assigned doctor's queue.
                    <strong> Rejecting</strong> cancels the appointment and
                    notifies the patient with your reason. Either way the
                    decision is recorded against the LOA number.
                </p>
            </div>
            ...
            >
                <div style={{ flex: 1, position: 'relative' }}>
                    ...
                    <input
                        type="search"
                        className="wc-input"
                        placeholder="Search by patient name, LOA number, HMO provider, or HMO ID…"
                        value={search}
                    ...
                    >
                        ...
                        >
                            {search || hmoFilter
                                ...
                                : 'All LOA requests have been reviewed.'}
                        </p>
                    </div>
                ) : (
                    <table
                ...
                {filtered.length > 0 && (
                    ...
                    >
                        ...
                        >
                            ...
                            {props.appointments.length} pending LOA{' '}
