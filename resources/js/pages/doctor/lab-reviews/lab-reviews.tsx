// resources/js/pages/doctor/lab-reviews/lab-reviews.tsx
// ─────────────────────────────────────────────────────────────────────────────
...
//
// Served by Doctor\LabReviewController@index; validating a result posts to
// doctor.lab-reviews.validate, which hands off to LabResultService::review().
...
import type { LabReviewRow } from './lab-reviews-data';

interface PageData extends PageProps {
    results: LabReviewRow[];
...
export default function LabReviewsPage({ results }: PageData): ReactElement {
    const meta = labReviewsMeta;

    // ── Modal state ────────────────────────────────────────────────────────────
    const [selectedResult, setSelectedResult] = useState<LabReviewRow | null>(
        null,
    );
    ...
    function handleValidate(id: string, notes: string): void {
        router.post(
            `/doctor/lab-reviews/${id}/validate`,
            { interpretation: notes },
            {
                preserveScroll: true,
                onSuccess: () => setSelectedResult(null),
            },
        );
    }
    ...
    return (
        <DashboardLayout activeId={meta.activeNavId}>
            ...
            <LabSubmissionsCard items={results} onReview={handleOpenModal} />
