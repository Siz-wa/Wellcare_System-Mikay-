  ...
  return (
    <div
      ...
      style={{
        ...
        position: "relative",
        boxShadow: hovered ? "0 12px 32px -6px rgba(59,130,246,0.18)" : "none",
    ...
    >
      ...
      <p style={{ margin: "0 0 var(--space-5)", fontSize: "var(--text-xs)", color: "var(--wc-gray-400)", fontWeight: 600 }}>
        {patient.patientId} · {patient.email}
      </p>

      {/* Active diagnoses warning */}
      {patient.activeDiagnoses > 0 && (
        <div style={{ marginBottom: "var(--space-3)", padding: "6px 12px", borderRadius: "8px", background: "#fff7ed", border: "1px solid #fed7aa", display: "inline-flex", alignItems: "center", gap: "6px" }}>
          <span style={{ fontSize: "11px", fontWeight: 700, color: "#c2410c" }}>
            {patient.activeDiagnoses} active {patient.activeDiagnoses === 1 ? "diagnosis" : "diagnoses"}
          </span>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: "var(--space-5)", marginBottom: "var(--space-5)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "6px", color: "var(--wc-gray-500)", fontSize: "var(--text-xs)", fontWeight: 600 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: "1px solid var(--wc-gray-100)", paddingTop: "var(--space-4)" }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", background: "#f0fdf4", color: "#16a34a", padding: "4px 10px", borderRadius: "12px", fontSize: "10px", fontWeight: 800, border: "1px solid #dcfce7" }}>
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#16a34a" strokeWidth="2.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>
          VERIFIED
        </span>
        <Link
          href={`/doctor/patient-records/${patient.id}`}
          style={{
            color: "var(--wc-blue-600)", fontSize: "var(--text-xs)", fontWeight: 700,
            textDecoration: "none",
            opacity: hovered ? 1 : 0,
...
export default function PatientRecordsPage(): ReactElement {
  ...
  function handleSearch(value: string): void {
    setSearch(value);
    router.get(window.location.pathname, { search: value, service: props.filters.service }, { preserveState: true, replace: true });
  }
  ...
  return (
    <DashboardLayout activeId={meta.activeNav}>
      ...
      <div style={{ display: "flex", gap: "var(--space-3)", marginBottom: "var(--space-6)" }}>
        <div className="wc-card" style={{ padding: "var(--space-4) var(--space-5)", display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
          <span style={{ fontSize: "var(--text-2xl)", fontWeight: 800, color: "var(--wc-blue-600)", fontFamily: "var(--font-display)" }}>{props.patients.total}</span>
          <span style={{ fontSize: "var(--text-sm)", fontWeight: 600, color: "var(--wc-gray-500)" }}>Total Patients</span>
        </div>
        <div className="wc-card" style={{ padding: "var(--space-4) var(--space-5)", display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", marginBottom: "var(--space-6)" }}>
        <div style={{ flex: 1, position: "relative" }}>
          ...
          <input
            ...
            style={{ paddingLeft: "calc(var(--space-4) + 22px)", fontSize: "var(--text-sm)", background: "var(--wc-white)", width: "100%" }}
          />
        </div>
      </div>
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "var(--space-5)" }}>
          ...
          <span style={{ fontSize: "var(--text-xs)", color: "var(--wc-gray-400)", fontWeight: 600 }}>
        ...
        {props.patients.data.length === 0 ? (
          <div className="wc-card" style={{ padding: "var(--space-12)", textAlign: "center" }}>
            <p style={{ margin: 0, fontSize: "var(--text-base)", color: "var(--wc-gray-400)" }}>No patients found.</p>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "var(--space-6)" }}>
            {props.patients.data.map(p => <RecordCard key={p.id} patient={p} />)}
          </div>
        )}
        ...
        {props.patients.last_page > 1 && (
          <div style={{ display: "flex", justifyContent: "center", gap: "var(--space-2)", marginTop: "var(--space-8)" }}>
            {Array.from({ length: props.patients.last_page }, (_, i) => i + 1).map(page => (
              <button
                key={page}
                onClick={() => router.get(window.location.pathname, { ...props.filters, page }, { preserveState: true })}
                style={{
                  width: 36, height: 36, borderRadius: "var(--radius-lg)", border: "1px solid var(--wc-gray-200)",
                  background: page === props.patients.current_page ? "var(--wc-blue-600)" : "#fff",
                  color:      page === props.patients.current_page ? "#fff" : "var(--wc-gray-600)",
