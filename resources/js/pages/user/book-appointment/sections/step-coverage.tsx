...
/** One doctor's capacity for a single date, from /appointments/doctor-availability. */
interface DoctorDayAvailability {
    /** Free time slots; null when the doctor has no schedule that day. */
    slots: number | null;
    /** Patients the daily cap still permits. */
    remaining: number | null;
    /** The doctor's configured daily patient cap. */
    cap: number | null;
}
...
export default function StepCoverage({
    ...
    // `slots` is null when the doctor has no schedule that day — distinct from 0,
        ...
        fetch(`/appointments/doctor-availability?date=${data.appointmentDate}`)
            ...
            .then((d) => {
                ...
                for (const [id, info] of Object.entries(d.availability ?? {})) {
                    const i = info as any;
                    map[Number(id)] = {
                        slots: i.no_schedule ? null : (i.available_slots ?? 0),
                        remaining: i.slots_remaining ?? null,
                        cap: i.daily_cap ?? null,
                    };
                }
        ...
        fetch(
            ...
            .then((d) => {
                ...
                if (
                    ...
                    !(d.slots ?? []).includes(data.appointmentTime)
                ) {
                    setData('appointmentTime', '');
                }
            })
    ...
    const totalPages = Math.max(
        1,
        Math.ceil(filteredDoctors.length / DOCS_PER_PAGE),
    );
    ...
    // Changing the service can filter out an already-picked doctor. Clear the
    // stale selection in an effect — doing it during render mutates form state
                    ...
                    >
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onPaste={makePasteHandler(sanitizeHmoId, (v) =>
                                    setData('hmoId', v),
                                )}
                            />
                            <div
                ...
                <div>
                    ...
                    >
                        ...
                        {pagedDoctors.map((doc) => {
                            ...
                            const dayInfo = data.appointmentDate
                                ? availability[doc.id]
                                : undefined;
                            const rawSlot = dayInfo?.slots;
                            const hasFetchedDoc =
                                Boolean(data.appointmentDate) &&
                                dayInfo !== undefined;
                            const slotCount = rawSlot ?? -1;
                            const isFullyBooked =
                                hasFetchedDoc && rawSlot === 0;
                            const hasAvailData =
                                hasFetchedDoc && rawSlot !== null;
                            const remaining = dayInfo?.remaining ?? null;
                            const dailyCap = dayInfo?.cap ?? null;

                            return (
                                <button
                                    key={doc.id}
                                    type="button"
                                    // The clinic spec is explicit: an unavailable doctor's booking
                                    // option must be disabled, not merely dimmed.
                                    ...
                                    style={{
                                        ...
                                        boxShadow: isSelected
                                            ? `0 0 0 4px ${isFullyBooked ? 'rgba(249,115,22,0.1)' : 'rgba(37,99,235,0.08)'}`
                                            : 'none',
                                    }}
                                >
                                    {/* Badge */}
                                    ...
                                    {/* The daily patient cap is the real constraint, so show that
                      rather than the raw slot count — a doctor can have twelve
                      free hours and still only take one more patient. */}
                                    {!isFullyBooked &&
                                        hasAvailData &&
                                        slotCount > 0 && (
                                            <span
                    ...
                    {!data.appointmentDate && (
                        ...
                        >
                            Select a date in Step 2 to see real-time
                            availability.
                        </p>
                    )}
                </div>
