...
import {
  LayoutDashboard, CalendarCheck2, Users,
  MessageSquare, FlaskConical, FolderOpen, Settings, LogOut, ChevronRight, Stethoscope,
} from "lucide-react";
...
export function SidebarLogo(): ReactElement {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "24px 24px 20px" }}>
      <div style={{ width: 40, height: 40, borderRadius: 12, background: BRAND, boxShadow: ACTIVE_SHADOW, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
         <svg
      ...
      <div style={{ lineHeight: 1 }}>
        <p style={{ margin: 0, fontSize: "1.05rem", fontWeight: 800, color: "#1e293b", fontFamily: "var(--font-display)", letterSpacing: "-0.02em" }}>WELLCARE</p>
        <p style={{ margin: 0, fontSize: "0.625rem", fontWeight: 800, color: BRAND, fontFamily: "var(--font-display)", letterSpacing: "0.15em" }}>CLINICS & LABORATORY</p>
      </div>
    </div>
  );
}

function NavLink({ item, active }: { item: NavItem; active: boolean }): ReactElement {
  ...
  return (
    <Link
      ...
      style={{
        ...
        padding: "10px 16px", borderRadius: 16, fontSize: "var(--text-sm)",
        fontWeight: active ? 600 : 500, textDecoration: "none",
        fontFamily: "var(--font-sans)",
        transform: hovered && !active ? "translateX(4px)" : "translateX(0)",
        transition: "transform 180ms cubic-bezier(0.16,1,0.3,1), background 150ms ease, color 150ms ease, box-shadow 150ms ease",
    ...
    >
      <span style={{ flexShrink: 0, display: "flex", opacity: active ? 1 : hovered ? 1 : 0.65 }}>{ICON_MAP[item.iconKey]}</span>
      <span style={{ flex: 1 }}>{item.label}</span>
      {active && <span style={{ display: "flex", opacity: 0.7 }}><ChevronRight size={14} strokeWidth={2.5} /></span>}
    </Link>
  );
}
  ...
  return (
    <Link
      ...
      style={{
        ...
        padding: "10px 16px", borderRadius: 16, fontSize: "var(--text-sm)",
        fontWeight: 500, textDecoration: "none", border: "none", cursor: "pointer",
      ...
      onMouseLeave={() => setHovered(false)}
    >
      <span style={{ flexShrink: 0, display: "flex", opacity: hovered ? 1 : 0.55 }}><LogOut size={17} strokeWidth={1.8} /></span>
      Logout
    </Link>
  );
}

interface PatientAppSidebarProps { activeId: string; }

export function PatientAppSidebar({ activeId }: PatientAppSidebarProps): ReactElement {
  return (
    <aside style={{
      ...
      <nav style={{ flex: 1, padding: "8px 16px", display: "flex", flexDirection: "column", overflowY: "auto" }}>
        {navGroups.map((group, gi) => (
          <div key={group.groupLabel} style={{ marginBottom: gi < navGroups.length - 1 ? 16 : 0 }}>
            <p style={{ margin: "0 0 6px", padding: "0 4px", fontSize: "10px", fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.15em", fontFamily: "var(--font-sans)" }}>
              {group.groupLabel}
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {group.items.map(item => <NavLink key={item.id} item={item} active={item.id === activeId} />)}
            </div>
          </div>
        ))}
      </nav>

      <div style={{ borderTop: "1px solid #f1f5f9", padding: "12px 16px 16px", display: "flex", flexDirection: "column", gap: 2 }}>
        <LogoutButton />
      </div>
    </aside>
  );
