...
import { labSubmissions, labReviewsMeta }   from "../lab-reviews-data";
...
const STATUS_CONFIG: Record<LabResultStatus, { label: string; bg: string; color: string }> = {
  pending:  { label: "Pending",  bg: "var(--wc-blue-50)",  color: "var(--wc-blue-600)" },
...
function SubmissionRow({ item, isLast }: { item: LabSubmission; isLast: boolean }): ReactElement {
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLDivElement).style.background = "var(--wc-gray-50)";
      }}
      onMouseLeave={(e) => {
    ...
    >
      ...
      {/* Review link — matches image exactly */}
      <a
        href={`/lab-reviews/${item.id}`}
        style={{
          flexShrink:     0,
          fontSize:       "var(--text-sm)",
          fontWeight:     600,
        ...
        onMouseEnter={(e) => {
          (e.currentTarget as HTMLAnchorElement).style.background = "var(--wc-blue-50)";
          (e.currentTarget as HTMLAnchorElement).style.color      = "var(--wc-blue-600)";
        }}
        onMouseLeave={(e) => {
          (e.currentTarget as HTMLAnchorElement).style.background = "transparent";
          (e.currentTarget as HTMLAnchorElement).style.color      = "var(--wc-sky-500)";
        }}
      >
        {meta.reviewLabel}
      </a>
...
export function LabSubmissionsCard(): ReactElement {
  const meta = labReviewsMeta;
  ...
  return (
    ...
    >
      ...
      {/* Rows */}
      <div>
        {labSubmissions.map((item, i) => (
          <SubmissionRow
            key={item.id}
            item={item}
            isLast={i === labSubmissions.length - 1}
          />
        ))}
      </div>
    </div>
