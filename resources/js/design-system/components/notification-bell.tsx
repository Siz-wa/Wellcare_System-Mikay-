...
function NotifIcon({ type }: { type: string }): ReactElement {
  const icons: Record<string, ReactElement> = {
    ...
    "clipboard-check": (
      <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/><polyline points="9 12 11 14 15 10"/>
      </svg>
    ),
  };
  ...
  const color = iconColors[type] ?? "var(--wc-blue-600)";
  const icon  = icons[type.replace(/_/g, "-").split("-").slice(1).join("-")] ?? icons["calendar"];
...
function NotificationDropdown({
  ...
  function dismiss(e: React.MouseEvent, id: string): void {
    e.stopPropagation();
    router.delete(`/notifications/${id}`, { preserveState: true, preserveScroll: true } as any);
  }
    ...
    >
      ...
      <div style={{
        ...
        {unreadCount > 0 && (
          <button
            onClick={markAllRead}
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: "var(--text-xs)", fontWeight: 700, color: "var(--wc-blue-600)", padding: 0 }}
          >
            Mark all read
          </button>
        )}
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map(n => (
            <div
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.background = n.read ? "var(--wc-gray-50)" : "#e0edff"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = n.read ? "var(--wc-white)" : "var(--wc-blue-50)"; }}
            >
      ...
      {notifications.length > 0 && (
        <div style={{
          ...
          <button
            onClick={() => router.delete('/notifications', { preserveState: true, preserveScroll: true } as any)}
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: "var(--text-xs)", color: "var(--wc-gray-400)", fontWeight: 600, padding: 0 }}
          >
