...
// ─── Styles ───────────────────────────────────────────────────────────────────
// All .wc-btn, .wc-nav-link classes → resources/css/components.css
// All --wc-* tokens                 → resources/css/tokens.css
// New .wc-navbar__* classes         → resources/css/components.css
//   (see layout-classes.css for the additions to paste in)

// ─── Logo ─────────────────────────────────────────────────────────────────────
// Exported so Footer.tsx can reuse it — single source of truth.
export function WellcareLogo({ dark = false }: { dark?: boolean }) {
  return (
    <a href="/" className="wc-logo">
      <div className="wc-logo__icon">
        {/* viewBox cropped to actual shape bounds so it fills and centers in the tile */}
        <svg
...
export default function Navbar({ active = "Home" }: NavbarProps) {
  ...
  return (
    <>
      {/* ── Header bar ────────────────────────────────────────────────────── */}
      <header className={`wc-navbar${scrolled ? " wc-navbar--scrolled" : ""}`}>
        <div className="wc-navbar__inner">

          <WellcareLogo />

          {/* Desktop links — hidden on mobile via CSS */}
          <nav className="wc-navbar__links" aria-label="Main navigation">
            {NAV_LINKS.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className={`wc-navbar__nav-link${active === link.label ? " wc-navbar__nav-link--active" : ""}`}
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="wc-navbar__actions">
            {/* Primary CTA — reuses existing .wc-btn classes from components.css */}
            <a
              href="/book"
              className="wc-btn wc-btn-primary wc-btn-md wc-btn-pill wc-navbar__cta"
            >
              Book Appointment
            </a>
      ...
      {/* ── Mobile drawer ─────────────────────────────────────────────────── */}
      {mobileOpen && (
        <div id="wc-mobile-drawer" className="wc-navbar__mobile-drawer" role="navigation" aria-label="Mobile navigation">
          {NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className={`wc-navbar__nav-link wc-navbar__nav-link--mobile${active === link.label ? " wc-navbar__nav-link--active" : ""}`}
            >
              {link.label}
            </a>
          ))}
          {/* Reuses .wc-btn-primary from components.css */}
          <a href="/book" className="wc-btn wc-btn-primary wc-btn-lg wc-btn-pill">
            Book Appointment
          </a>
        </div>
      )}
