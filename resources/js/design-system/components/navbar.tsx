...
interface WellcareLogoProps { dark?: boolean; }
  ...
  return (
    <Link href={home.url()} className="wc-logo" aria-label="Wellcare Clinics — Home">
      <div className="wc-logo__icon">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="371 196 647 635" width="26" height="26" xmlSpace="preserve" aria-hidden="true" focusable="false">
...
function getDashboardUrl(roles: string[]): string {
  if (roles.includes("doctor")) return "/doctor/appointments";
  if (roles.includes("hr"))     return "/hr/dashboard";
  if (roles.includes("admin"))  return "/hr/dashboard";
...
interface NavbarProps { active?: NavLabel; }
...
export default function Navbar({ active }: NavbarProps) {
  ...
  const { auth } = props;
  const isAuthenticated = !!auth?.user;
  const resolvedActive  = active ?? NAV_LINKS.find(l => l.href === url)?.label;
      ...
      <header className={`wc-navbar${scrolled ? " wc-navbar--scrolled" : ""}`}>
        <div className="wc-navbar__inner">
          ...
          <nav className="wc-navbar__links" aria-label="Main navigation">
            {NAV_LINKS.map(link => (
              <Link
                ...
                aria-current={resolvedActive === link.label ? "page" : undefined}
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="wc-navbar__actions">
            ...
            <span className="wc-navbar__cta"><CtaButton size="md" /></span>
            <button
              type="button"
