...
interface StatCardProps {
  ...
  iconVariant?: 'primary' | 'sky' | 'success' | 'warning' | 'error' | 'purple' | 'cyan' | 'emerald';
  className?: string;
}
  ...
  <div className={`wc-card wc-card-body ${className}`}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 'var(--space-4)' }}>
      <div className="wc-stat">
        ...
        {delta && (
          <span className={`wc-stat-delta wc-stat-delta-${delta.direction}`}>
      ...
      {icon && (
        <div className={`wc-icon-tile wc-icon-tile-lg ${iconVariantMap[iconVariant]}`}>
          {icon}
        </div>
      )}
    </div>
