 ...
 * DoctorProfileSeeder
 * ──────────────────────────────────────────────────────────────────────────────
 * One-time seeder to bootstrap doctor_profiles from the hardcoded doctorsData.ts
 * array. Run this AFTER the migration.
 *
 * For each doctor:
 *   1. Finds or creates a User account (email = slugified name @wellcare.ph).
 *   2. Assigns the "doctor" Spatie role.
 *   3. Creates the DoctorProfile row.
 *
 * Idempotent — safe to run multiple times (uses firstOrCreate).
 *
 * Run: php artisan db:seed --class=DoctorProfileSeeder
 */
